...
<a name="show"></a>
# **show**
> IntegrationSlack show(integrationId, include)
...
String include = "include_example"; // String | Related objects that can be included in the response:  integration See Including Objects for more information.
try {
    IntegrationSlack result = apiInstance.show(integrationId, include);
    System.out.println(result);
} catch (ApiException e) {
    System.err.println("Exception when calling IntegrationsSlackApi#show");
    e.printStackTrace();
}
```
