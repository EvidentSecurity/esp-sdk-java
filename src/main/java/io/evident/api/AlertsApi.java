...
public interface AlertsApi {
  ...
  Call<PaginatedCollection> listComplianceControls(
    @retrofit2.http.Path("alert_id") Integer alertId, @retrofit2.http.Query("include") String include, @retrofit2.http.Field("page") String page
  );
   ...
   * @param include Related objects that can be included in the response:  external_account, region, signature, custom_signature, suppression, metadata, attribution, cloud_trail_events, tags, compliance_controls See Including Objects for more information. (optional)
  ...
  Call<PaginatedCollection> listForReport(
    @retrofit2.http.Path("report_id") Integer reportId, @retrofit2.http.Query("include") String include, @retrofit2.http.Field("filter") Map<String, String> filter, @retrofit2.http.Field("page") String page
  );
